...
class BoardCategoryController {
    ...
    public function index(\Base $f3): void {
        ...
        $category = new BoardCategory($f3->get('DB'));
        $categories = $category->findByUser((int) $user->id);
        $this->json($f3, ['board_categories' => $categories]);
    }
    ...
    public function create(\Base $f3): void {
        ...
        if ($parentId > 0) {
            $parent = new BoardCategory($f3->get('DB'));
        ...
        $this->json($f3, ['board_category' => $category->toArray()], 201);
    }
    ...
    public function update(\Base $f3, array $params): void {
        ...
        $this->json($f3, ['board_category' => $category->toArray()]);
    }
    ...
    public function delete(\Base $f3, array $params): void {
        ...
        $db = $f3->get('DB');
        $db->exec('UPDATE board_categories SET parent_id = ? WHERE parent_id = ?', [$parentId, $id]);
        if ($parentId !== null) {
        ...
        $this->json($f3, ['message' => 'Kategoria tablicy została usunięta.']);
    }
}
