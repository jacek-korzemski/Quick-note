...
class CategoryController {
    ...
    public function index(\Base $f3): void {
        ...
        $this->json($f3, ['categories' => $categories]);
    }
    ...
    public function create(\Base $f3): void {
        ...
        $this->json($f3, ['category' => $category->toArray()], 201);
    }
    ...
    public function update(\Base $f3, array $params): void {
        ...
        $this->json($f3, ['category' => $category->toArray()]);
    }
    ...
    public function delete(\Base $f3, array $params): void {
        ...
        $db = $f3->get('DB');
        $db->exec('UPDATE categories SET parent_id = ? WHERE parent_id = ?', [$parentId, $id]);
        if ($parentId !== null) {
            $db->exec('UPDATE notes SET category_id = ? WHERE category_id = ?', [$parentId, $id]);
        } else {
            $db->exec('UPDATE notes SET category_id = NULL WHERE category_id = ?', [$id]);
        }
        $category->erase();
